...
const App: React.FC = () => {
  ...
  // Navegação segura que suporta Vercel e ambientes restritos
  const navigateTo = (path: string) => {
    try {
      window.history.pushState({}, '', path);
    } catch (e) {
      console.warn("Ambiente restrito: não foi possível atualizar a URL física, usando navegação lógica.");
    }
    handleNavigation();
  };
  ...
  const handleNavigation = async () => {
    ...
    // Fallback para links antigos com hash (#/)
    ...
    // Decisão de View baseada no path
    if (currentPath.includes('/admin-secret')) {
      if (session && session.user.id === ADMIN_UID) setView('admin');
      else setView('login');
    } else if (currentPath === '/roleta' || currentPath.endsWith('/roleta')) {
      setView('roleta');
    } else if (currentPath === '/bonusaleatorio' || currentPath.endsWith('/bonusaleatorio')) {
      setView('bonusaleatorio');
    } else if (currentPath === '/5debonus' || currentPath.endsWith('/5debonus')) {
      setView('5debonus');
    } else {
      setView('public');
    }
  ...
  const categories = useMemo(() => {
    ...
    links.forEach(l => {
      ...
      if (!forbidden.includes(lowerName) && !foundCats.includes(name)) {
        foundCats.push(name);
      }
    });
      ...
      <div className="min-h-screen bg-black text-white relative font-sans overflow-x-hidden pb-20">
        ...
        <main className="relative z-10 max-w-lg mx-auto px-6 py-16 flex flex-col items-center text-center">
          <header className="mb-12 flex flex-col items-center">
            <div className="w-32 h-32 p-1 rounded-full bg-gradient-to-br from-purple-400 to-indigo-600 relative shadow-2xl overflow-hidden mb-8">
            ...
            <h1 className="text-5xl font-black uppercase italic tracking-tighter text-shimmer mb-2">{title || brand.roletaTitle}</h1>
            <h2 className="text-xl font-black uppercase italic tracking-tighter text-white opacity-70">{tagline || brand.roletaTagline}</h2>
          </header>
          <div className="w-full space-y-4 mb-16">
            {pageLinks.length > 0 ? pageLinks.map(link => <LinkButton key={link.id} link={link} />) : <div className="glass-card p-10 rounded-[2rem] opacity-30 text-[10px] font-black uppercase">Página Vazia</div>}
          </div>
          <button onClick={() => navigateTo('/')} className="fixed bottom-6 right-6 w-14 h-14 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center shadow-2xl transition-all active:scale-90"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>
        </main>
      </div>
    );
  };
  ...
  if (view === 'login') return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-black relative">
      ...
      <div className="w-full max-w-sm glass-card p-10 rounded-[3.5rem] z-10 border border-white/10 text-center">
        <h2 className="text-3xl font-black uppercase text-shimmer mb-10">Admin Master</h2>
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" disabled={loginLoading} className="w-full py-5 bg-yellow-500 text-black font-black rounded-2xl uppercase tracking-widest">{loginLoading ? 'Carregando...' : 'Acessar'}</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-black text-white relative font-sans overflow-x-hidden pb-20">
      ...
      <main className="relative z-10 max-w-lg mx-auto px-6 py-16 flex flex-col items-center">
        <header className="text-center mb-12 flex flex-col items-center">
          <div className="w-32 h-32 p-1 rounded-full bg-gradient-to-br from-yellow-200 to-yellow-600 shadow-2xl overflow-hidden mb-8 animate-float">
            <img src={brand.logoUrl} className="w-full h-full rounded-full object-cover border-[5px] border-black" alt="Logo" />
          </div>
          <div className="flex items-center gap-3">
            <h1 className="text-4xl font-black uppercase italic text-shimmer">{brand.name}</h1>
          ...
          <p className="text-[11px] font-bold text-white uppercase tracking-[0.3em] mt-5 border-t border-white/10 pt-4 px-6">{brand.tagline}</p>
        </header>
        {categories.length > 1 && (
        ...
        <div className={`w-full space-y-6 transition-all duration-300 ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          {activeCategory.trim().toLowerCase() === 'listas' && (
            <button onClick={() => navigateTo('/roleta')} className="relative w-full p-5 rounded-[2.2rem] flex items-center gap-5 border-b-[6px] neon-purple-btn transform hover:scale-[1.03] transition-all">
              <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 overflow-hidden p-1"><img src={brand.logoUrl} className="w-full h-full object-cover rounded-xl" alt="Logo" /></div>
              <div className="flex-grow text-left"><h3 className="text-[16px] font-black uppercase text-purple-100">{brand.roletaTitle}</h3><p className="text-[11px] opacity-60">{brand.roletaTagline}</p></div>
              <svg className="w-6 h-6 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M13 5l7 7-7 7" /></svg>
            </button>
          )}
        ...
        <footer className="w-full text-center mt-20 space-y-10">
          ...
          <p className="text-[9px] text-gray-500 font-black uppercase tracking-[0.4em]">{brand.footerText?.split('EXTRAS:')[0]?.split('ORDER:')[0] || `${brand.name} © 2025`}</p>
        </footer>
      </main>
    </div>
  );
