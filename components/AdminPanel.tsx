...
const AdminPanel: React.FC = () => {
  ...
  const fetchBrand = async () => {
    ...
    if (data) {
      ...
      setBrand({ 
        ...data, 
        extraPages, 
        roletaTitle: data.roleta_title, 
        roletaTagline: data.roleta_tagline, 
        roletaLogoUrl: data.roleta_logo_url, 
        roletaBadgeText: data.roleta_badge_text, 
  ...
  const handleSaveBrand = async (e: React.FormEvent) => {
    ...
    alert("Visual Global atualizado!");
    setLoading(false);
  };
      ...
      <div className="grid grid-cols-3 gap-3 mb-12">
        {(['links', 'social', 'brand'] as const).map(m => (
          <button key={m} onClick={() => setActiveMenu(m)} className={`py-5 rounded-[1.8rem] text-[10px] font-black uppercase border transition-all ${activeMenu === m ? 'bg-yellow-500 text-black border-yellow-500 shadow-lg' : 'bg-white/5 text-gray-400 border-white/5'}`}>
            {m === 'links' ? '🎰 Links' : m === 'social' ? '📱 Social' : '🎨 Visual'}
          </button>
        ))}
      </div>
        ...
        <form onSubmit={handleSaveBrand} className="space-y-8 animate-fade-in">
          <div className="flex gap-2">
             {['geral', 'roleta', 'bonus'].map((t) => (
                <button key={t} type="button" onClick={() => setActiveBrandTab(t as any)} className={`flex-1 py-4 rounded-2xl text-[8px] font-black uppercase border transition-all ${activeBrandTab === t ? 'bg-white text-black border-white' : 'bg-white/5 border-white/5 text-gray-500'}`}>{t === 'geral' ? 'Configuração Global' : t === 'roleta' ? 'Página Roleta' : 'Página Bônus'}</button>
             ))}
          </div>
          ...
          <div className="bg-[#0f0f0f] p-8 rounded-[3rem] border border-white/5 space-y-6">
            {activeBrandTab === 'geral' && (
              <div className="space-y-6">
                ...
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.name} onChange={e => setBrand({...brand, name: e.target.value})} placeholder="Nome Marca" />
                ...
                <label className="text-[9px] font-black uppercase text-gray-500 px-2">Efeito Visual Global</label>
                <select className="w-full bg-black p-5 rounded-2xl border border-white/10 text-sm" value={brand.effect} onChange={e => setBrand({...brand, effect: e.target.value as any})}><option value="scanner">Scanner</option><option value="money">Dinheiro</option><option value="gold-rain">Chuva de Ouro</option><option value="space">Estrelas</option><option value="aurora">Aurora</option><option value="lightning">Raios</option><option value="matrix">Matrix</option><option value="fire">Fogo</option><option value="glitch">Glitch</option><option value="confetti">Confetti</option><option value="none">Nenhum</option></select>
              </div>
            )}

            {activeBrandTab === 'roleta' && (
              <div className="space-y-6">
                <button type="button" onClick={() => roletaLogoInputRef.current?.click()} className="w-full p-4 bg-black border border-white/10 rounded-2xl text-[8px] font-black uppercase">Logo da Página Roleta</button>
                <input type="file" ref={roletaLogoInputRef} onChange={e => handleFileUpload(e, 'roleta_logo')} className="hidden" />
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.roletaTitle} onChange={e => setBrand({...brand, roletaTitle: e.target.value})} placeholder="Título Roleta" />
                ...
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.roletaBadgeText} onChange={e => setBrand({...brand, roletaBadgeText: e.target.value})} placeholder="Badge VIP" />
              </div>
            )}

            {activeBrandTab === 'bonus' && (
              <div className="space-y-6">
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.extraPages?.bonusaleatorio?.title} onChange={e => updateExtraPage('bonusaleatorio', 'title', e.target.value)} placeholder="Título Bônus Surpresa" />
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.extraPages?.bonusaleatorio?.tagline} onChange={e => updateExtraPage('bonusaleatorio', 'tagline', e.target.value)} placeholder="Subtítulo" />
                <input className="w-full bg-black p-5 rounded-2xl border border-white/10 outline-none text-sm" value={brand.extraPages?.bonusaleatorio?.badge} onChange={e => updateExtraPage('bonusaleatorio', 'badge', e.target.value)} placeholder="Badge" />
              </div>
            )}
      ...
      {activeMenu === 'links' && (
        <div className="space-y-8 animate-fade-in">
          <div className="flex flex-wrap gap-2 p-4 bg-white/5 border border-white/5 rounded-[2.5rem]">
            {useMemo(() => Array.from(new Set(links.map(l => (l.category || 'Página 1').trim()))).length > 0 ? Array.from(new Set(links.map(l => (l.category || 'Página 1').trim()))) : ['Página 1'], [links]).map((c) => (
